...
这是基于Vant的组件,使用前请确保安装了Vant,并Use了Loading组件
    ...
    type            loading类型,同Vant的Loading组件,默认spinner,可选circular,   String
    color           loading颜色,同Vant,默认#c9c9c9, String
    size            loading尺寸,同Vant,默认30px,    String
    show            是否显示此组件,默认false,   Boolean
    position        组件在页面的位置,基于定位,要用这个属性须确保外层无相对定位的父元素,默认值center,可选static,   String
    modal           是否存在遮罩层,true,可选false,   Boolean
